...
const sortBy = (items, fn) => items.sort((a, b) => fn(a) - fn(b));
...
export default function ComparePricing({ hints, plans, preview }) {
  const limits = plans[0].attributes.limits.filter(
    (l) => l.id !== 'workflows_count',
  );

  const enterpriseOnlyLimits = hints
    .filter((h) => !limits.find((l) => l.id === h.apiId))
    .map((h) => ({ id: h.apiId, type: 'boolean_system_limit' }));

  const limitsByGroup = groupBy(
    sortBy(limits, (limit) => {
      const hint = hints.find((h) => h.apiId === limit.id);
      return hint?.position || 1000;
    }),
    (limit) =>
  ...
  limitsByGroup['boolean_system_limit'] = [
    ...limitsByGroup['boolean_system_limit'].sort(
      (l1, l2) =>
        plans.filter(
          (p) => p.attributes.limits.find((l) => l2.id === l.id).available,
        ).length -
        plans.filter(
          (p) => p.attributes.limits.find((l) => l1.id === l.id).available,
        ).length,
    ),
        ...
        <table className={s.table}>
          <tbody>
            ...
            {[
              ...
              .map((type) => [type, limitsByGroup[type]])
              .map(([group, limits]) => (
