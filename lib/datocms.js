...
export function performQuery(query, propsToVars, convertData) {
  return async props => {
    ...
    const { data } = await apolloClient.query({
      query,
      variables:
        typeof propsToVars === 'function' ? propsToVars(props) : propsToVars,
    });

    return typeof convertData === 'function' ? convertData(data) : data;
  };
}
...
export function gqlStaticProps(query, paramsToVars, dataToProps) {
  return performQuery(
    ...
    data => ({
      props: dataToProps === 'function' ? dataToProps(data) : data,
    }),
  );
}
