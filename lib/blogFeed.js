          ...
          'content:encoded': article.content
            .map((block) => {
              ...
              if (block._modelApiKey === 'internal_video') {
                return `<img src="${block.video.video.thumbnailUrl}?time=${
                  block.thumbTimeSeconds || block.video.video.duration / 2
                }&width=900" />`;
              }
              ...
              if (block._modelApiKey === 'demo') {
                return `
                  ...
                  <p>
                    <a href="${`https://dashboard.datocms.com/deploy?repo=${block.demo.githubRepo}`}">Deploy the demo project</a>
                  </p>
                `;
              }
